...
class CrossTrijoint(nn.Module):
    ...
    def __init__(self, opt, nb_classes, with_classif=False):
        ...
        if self.itm_loss_weight > 0:
            ...
            self.proj_title_cross = nn.Linear(self.recipe_embedding.hidden_size, self.dim_emb)
            self.proj_ingr_cross = nn.Linear(self.recipe_embedding.hidden_size, self.dim_emb)
            self.proj_instr_cross = nn.Linear(self.recipe_embedding.hidden_size, self.dim_emb)
        ...
        self.proj_title = nn.Linear(self.recipe_embedding.hidden_size, self.dim_emb)
        self.proj_ingr = nn.Linear(self.recipe_embedding.hidden_size, self.dim_emb)
        self.proj_instr = nn.Linear(self.recipe_embedding.hidden_size, self.dim_emb)
        

    def get_parameters_recipe(self):
        params = []
        params.append({'params': self.recipe_embedding.parameters()})
        #params.append({'params': self.title_embedding.parameters()})
        #params.append({'params': self.ingr_embedding.parameters()})
        #params.append({'params': self.insr_embedding.parameters()})
        if self.with_classif:
        ...
        params.append({'params': self.proj_title.parameters()})
        params.append({'params': self.proj_ingr.parameters()})
        params.append({'params': self.proj_instr.parameters()})
        if self.itm_loss_weight > 0:
            ...
            params.append({'params': self.proj_title_cross.parameters()})
            params.append({'params': self.proj_ingr_cross.parameters()})
            params.append({'params': self.proj_instr_cross.parameters()})
    ...
    def get_parameters_cross(self):
        ...
        if self.itm_loss_weight > 0:
            ...
            params.append({'params': self.proj_cross.parameters()})
            params.append({'params': self.proj_recipe_cross.parameters()})
            params.append({'params': self.proj_recipe_title.parameters()})
            params.append({'params': self.proj_recipe_ingr.parameters()})
            params.append({'params': self.proj_recipe_instr.parameters()})
            params.append({'params': self.proj_image_cross.parameters()})
            if self.cross_decoder_img:
                params.append({'params': self.proj_recipe_to_image.parameters()})
                params.append({'params': self.cross_decoder_image.parameters()})
    ...
    def forward_DE(self, batch):
        ...
        recipe_embedding, title_embedding, ingr_embedding, instr_embedding = self.recipe_embedding(batch['recipe'])
        
        out['recipe_feat'] = recipe_embedding[1]
        out['recipe_embedding'] = recipe_embedding[0]

        out['title_feat'] = title_embedding
        out['title_embedding'] = title_embedding.mean(1)

        out['ingr_feat'] = ingr_embedding
        out['ingr_embedding'] = ingr_embedding.mean(1)

        out['instr_feat'] = instr_embedding
        out['instr_embedding'] = instr_embedding.mean(1)

        #print("Line 185")
        #print(image_embedding.shape)
        #print(recipe_embedding[0].shape, recipe_embedding[1].shape)
        #print(title_embedding[0].shape, title_embedding[1].shape)
        #print(ingr_embedding[0].shape, ingr_embedding[1].shape)
        #print(instr_embedding[0].shape, instr_embedding[1].shape)
        ...
        if self.with_classif:
            ...
            out['title_classif'] = self.linear_classif(out['title_embedding'])
            out['insgr_classif'] = self.linear_classif(out['ingr_embedding'])
            out['instr_classif'] = self.linear_classif(out['instr_embedding'])
    ...
    def forward(self, batch):
        ...
        title_embed = out['title_embedding']
        ingr_embed = out['ingr_embedding']
        instr_embed = out['instr_embedding']
        title_embed = F.normalize(F.tanh(self.proj_title(title_embed)))
        ingr_embed = F.normalize(F.tanh(self.proj_ingr(ingr_embed)))
        instr_embed = F.normalize(F.tanh(self.proj_instr(instr_embed)))
        ...
        #print(ingr_embed.shape)
        #print(instr_embed.shape)
        
        if self.itm_loss_weight > 0:

            recipe_feat = out['recipe_feat']
            title_feat = out['title_feat']
            ingr_feat = out['ingr_feat']
            instr_feat = out['instr_feat']

            recipe_feat = self.proj_recipe_cross(recipe_feat)
            out['recipe_feat'] = recipe_feat
            title_feat = self.proj_title_cross(title_feat)
            out['title_feat'] = title_feat
            ingr_feat = self.proj_ingr_cross(ingr_feat)
            out['ingr_feat'] = ingr_feat
            instr_feat = self.proj_recipe_cross(instr_feat)
            out['instr_feat'] = instr_feat
            ...
            sim_title2recipe = title_embed @ recipe_embed.T # (bs1, bs2)
            sim_recipe2title = sim_title2recipe.T

            sim_ingr2recipe = ingr_embed @ recipe_embed.T # (bs1, bs2)
            sim_recipe2ingr = sim_ingr2recipe.T

            sim_instr2recipe = instr_embed @ recipe_embed.T # (bs1, bs2)
            sim_instr2title = sim_instr2recipe.T
        ...
        out['title_embedding'] = title_embed
        out['ingr_embedding'] = ingr_embed
        out['instr_embedding'] = instr_embed
